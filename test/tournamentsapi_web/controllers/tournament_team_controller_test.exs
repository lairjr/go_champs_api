defmodule TournamentsApiWeb.TournamentTeamControllerTest do
  ...
  def fixture(:tournament_team) do
    ...
    {:ok, tournament_team} = Tournaments.create_tournament_team(attrs)
    tournament_team
  end
  ...
  describe "index" do
    test "lists all tournament_teams", %{conn: conn} do
      conn = get(conn, Routes.tournament_team_path(conn, :index, "tournament-id"))
  ...
  describe "create tournament_team" do
    test "renders tournament_team when data is valid", %{conn: conn} do
      ...
      conn = get(conn, Routes.tournament_team_path(conn, :show, attrs.tournament_id, id))
  ...
  describe "update tournament_team" do
    ...
    test "renders tournament_team when data is valid", %{
      conn: conn,
      tournament_team: %TournamentTeam{id: id, tournament_id: tournament_id} = tournament_team
    } do
      conn =
        put(conn, Routes.tournament_team_path(conn, :update, tournament_id, tournament_team),
          tournament_team: @update_attrs
        )

      assert %{"id" => ^id} = json_response(conn, 200)["data"]

      conn = get(conn, Routes.tournament_team_path(conn, :show, tournament_id, id))
