defmodule TournamentsApiWeb.PhaseStandingsController do
  ...
  def index(conn, %{"phase_id" => phase_id}) do
    phase_standings = Phases.list_phase_standings(phase_id)
  ...
  def create(conn, %{
    ...
    phase_standings_params = Map.merge(phase_standings_params, %{"phase_id" => phase_id})
    ...
    with {:ok, %PhaseStandings{} = phase_standings} <-
      ...
      |> put_resp_header(
        "location",
        Routes.phase_standings_path(conn, :show, phase_id, phase_standings)
      )
  ...
  def show(conn, %{"id" => id, "phase_id" => phase_id}) do
    phase_standings = Phases.get_phase_standings!(id, phase_id)
