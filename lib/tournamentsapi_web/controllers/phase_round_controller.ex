defmodule TournamentsApiWeb.PhaseRoundController do
  ...
  def index(conn, %{"tournament_phase_id" => tournament_phase_id}) do
    phase_rounds = Phases.list_phase_rounds(tournament_phase_id)
  ...
  def create(conn, %{
    ...
    phase_round_params =
      Map.merge(phase_round_params, %{"tournament_phase_id" => tournament_phase_id})
    ...
    with {:ok, %PhaseRound{} = phase_round} <- Phases.create_phase_round(phase_round_params) do
      ...
      |> put_resp_header(
        "location",
        Routes.phase_round_path(conn, :show, tournament_phase_id, phase_round)
      )
  ...
  def show(conn, %{"id" => id, "tournament_phase_id" => tournament_phase_id}) do
    phase_round = Phases.get_phase_round!(id, tournament_phase_id)
